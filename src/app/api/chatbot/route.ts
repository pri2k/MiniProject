...
// Define types for Section and ScoredSection
interface Section {
  _id: string;
  section_text: string;
  keywords_found: string[];
...
interface Message {
  sender: 'user' | 'bot'; // Limiting sender to 'user' or 'bot'
  text: string;
}
...
export async function POST(req: Request) {
  try {
    ...
    const { message, history }: { message: string, history: Message[] } = await req.json(); // Use Message[] type
    console.log("📨 Message:", message);
    console.log("🕘 History:", history);
