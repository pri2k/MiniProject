...
export default function WordList({ words, foundWords, score, timeLeft }) {
    return (
        <div className="flex flex-col items-center relative lg:top-[70px] top-[-50px]">
        ...
        <div className="flex flex-wrap gap-2 max-w-xs justify-center">
            {words.map((word, i) => (
            <span key={i} className={`px-2 py-1 rounded-full border ${foundWords.includes(word) ? "bg-[#D7A529] text-white line-through" : "bg-white"}`}>
                {word}
            </span>
            ))}
        </div>
